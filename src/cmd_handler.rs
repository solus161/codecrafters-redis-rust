...
impl CmdHandler {
    ...
    fn cmd_acl(&mut self, opt: CmdArg, client_id: u64) -> Result<Option<RespType>, CustomError> {
        match opt {
            CmdArg::WhoAmI => self.cmd_acl_whoami(client_id),
            CmdArg::GetUser(username) => self.cmd_acl_getuser(client_id, username),
            CmdArg::SetUser { key, rules } => self.cmd_acl_setuser(key, rules),
        ...
        -> Result<Option<RespType>, CustomError>
    {
        ...
        let credential = binding.get_credential_mut(&username)
            .ok_or(CustomError::WrongUsernamePassword("Wrong username".to_string()))?;
        
        let _ = rules.drain(..).try_for_each(|r| -> Result<(), CustomError> {
            match r {
                AclRules::NoPass => {
                    credential.add_flag(AuthFlags::NoPass)
                        .then(|| {})
                        .ok_or(CustomError::InternalError("Error setting flag".to_string()))?;
                    Ok(())
                },
                AclRules::AddPassword(s) => {
                    credential.set_password(&s); 
                    Ok(())
                }
            }
        });
